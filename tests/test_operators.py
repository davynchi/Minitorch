...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert_close(sigmoid(0), .5)
    assert sigmoid(a) <= sigmoid(a + 1.)  # ??? How to make < ???
...
def test_symmetric() -> None:
    ...
    assert mul(5., 7.) == mul(7., 5.)
    assert mul(-5., -7.) == mul(-7., -5.)
    assert mul(0., 3.) == mul(3., 0.)
...
def test_distribute() -> None:
    ...
    assert mul(2., add(3., -3.)) == add(mul(2., 3.), mul(2., -3.))
    assert mul(1., add(3., 5.)) == add(mul(1., 3.), mul(1., 5.))
    assert mul(0., add(3., 5.)) == add(mul(0., 3.), mul(0., 5.))
    assert mul(2., add(3., 0.)) == add(mul(2., 3.), mul(2., 0.))
    assert mul(-2., add(-3., -5.)) == add(mul(-2., -3.), mul(-2., -5.))
...
def test_other() -> None:
    ...
    """
    assert mul(2., mul(-3., 5.)) == mul(mul(2., -3.), 5.)
    assert add(2., add(-3., 5.)) == add(add(2., -3.), 5.)
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    for i, j, k in zip(ls1, ls2, check):
        assert_close(i + j, k)
